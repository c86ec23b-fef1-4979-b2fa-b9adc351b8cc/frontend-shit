...
window.setupInput = () => {
    ...
    window.onkeydown = e => {
        ...
        if(e.keyCode === 9 || !isTyping && e.ctrlKey && e.metaKey) {
            e.preventDefault();

            if (!keyDownMap.get(e.keyCode)) Game.socket.send(new Uint8Array([0xA])); // switch team (TAB key)
        }
    
        if (e.keyCode === 32 && !keyDownMap.get(e.keyCode)) {
